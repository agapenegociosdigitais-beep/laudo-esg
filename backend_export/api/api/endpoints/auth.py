"""
Endpoints de autenticao: registro, login e dados do usurio atual.
"""
...
@router.post("/registrar", response_model=TokenResposta, status_code=status.HTTP_201_CREATED)
async def registrar_usuario(dados: UsuarioCriar, db: SessaoDB):
    """
    Cria uma nova conta na plataforma Eureka Terra.
    Retorna o token JWT para acesso imediato aps o registro.
    """
    ...
    if resultado.scalar_one_or_none():
        raise HTTPException(
            ...
            detail="Este e-mail j est cadastrado. Use outro e-mail ou faa login.",
        )
...
async def login(dados: LoginRequest, db: SessaoDB):
    """
    Autentica o usurio e retorna o token JWT de acesso.
    """
    resultado = await db.execute(select(Usuario).where(Usuario.email == dados.email))
...
async def obter_perfil(usuario: UsuarioAtual):
    """Retorna os dados do usurio autenticado."""
