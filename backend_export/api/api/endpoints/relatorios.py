"""
Endpoints para gerao e download de relatrios PDF.
"""
import uuid
...
async def gerar_relatorio(dados: RelatorioRequest, db: SessaoDB, usuario: UsuarioAtual):
    """
    Gera um relatrio PDF para uma anlise concluda.
    O PDF  salvo no servidor e um link de download  retornado.
    """
    # Verifica se a anlise existe e est concluda
    ...
    if not analise:
        raise HTTPException(
            ...
            detail="Anlise no encontrada.",
        )

    if analise.status != "concluido":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"A anlise ainda no foi concluda. Status atual: {analise.status}",
        )
    ...
    if not propriedade:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Propriedade da anlise no encontrada.",
        )
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Erro ao gerar relatrio: {str(e)}",
        )
...
async def download_relatorio(relatorio_id: uuid.UUID, db: SessaoDB, usuario: UsuarioAtual):
    """Faz o download do PDF do relatrio gerado."""
    ...
    if not relatorio:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Relatrio no encontrado ou sem permisso de acesso.",
        )
    ...
    if not caminho.exists():
        raise HTTPException(
            ...
            detail="Arquivo do relatrio no encontrado no servidor.",
        )
