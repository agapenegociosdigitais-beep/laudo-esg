"""
Servio de Conformidade Socioambiental - Eureka Terra
Critrios baseados no Protocolo de Monitoramento de Fornecedores de Gado da Amaznia
Inclui verificaes para pecuria E soja (dupla conformidade)
"""
import hashlib
import logging
import re
...
async def verificar_assentamentos(
    ...
    try:
        async with httpx.AsyncClient(timeout=20.0, verify=False, headers=HEADERS) as client:
            ...
            sobreposicao = len(feats) > 0
            nomes = [f.get("properties", {}).get("nom_proje", "") for f in feats[:3]]
...
# ?Trabalho Escravo (Portal da Transparncia) ?
async def verificar_trabalho_escravo(
    ...
    """
    Consulta a Lista Suja do Trabalho Escravo via Portal da Transparncia.
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0, verify=False, headers=HEADERS) as client:
            ...
            return {
                ...
                "verificado": True,
                "fonte": "Portal da Transparncia / MTE",
            }
    except Exception as e:
        logger.warning(f"MTE Lista Suja indisponvel: {e}")
        return {
            "trabalho_escravo": False,
            "verificado": False,
            "fonte": "MTE (indisponvel)",
        }

# ?Balano Ambiental (Cdigo Florestal Lei 12.651/2012) ?
def calcular_balanco_ambiental(
    ...
    pct_rl = 0.80 if "Amaznia" in bioma else 0.20
...
async def obter_geometria_sicar(car_numero: str) -> Optional[dict]:
    """Busca geometria real do imvel no SICAR via GeoServer WFS."""
    try:
...
async def analisar_conformidade_completa(
    ...
    2. Usa geometria em todas as verificaes espaciais
    3. Retorna laudo consolidado
    """
    import asyncio
    from app.services.embargos_service import verificar_embargos_ibama

    logger.info(f"Iniciando anlise completa: {car_numero}")
    ...
    if quilombolas.get("sobreposicao"):
        problemas.append("Sobreposio com territrio quilombola")
    if assentamentos.get("sobreposicao"):
        problemas.append("Sobreposio com assentamento INCRA")
