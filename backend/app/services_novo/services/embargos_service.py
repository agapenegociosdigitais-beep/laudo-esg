...
class ResultadoEmbargo:
    ...
    def para_dict(self) -> Dict[str, Any]:
        ...
        Mantm compat com o frontend (campo 'embargado' lido em ComplianceStatus.tsx).
        """
...
async def verificar_marco_ue_prodes(
    ...
    """
    Verifica desmatamento aps 31/12/2020 (Marco EUDR/UE).

    Endpoint confirmado: https://terrabrasilis.dpi.inpe.br/geoserver/ows
    Camadas disponveis (verificadas via GetCapabilities):
      - prodes-legal-amz:yearly_deforestation  (Amaznia Legal)
    ...
    # Escolhe camadas por bioma ?usa Legal Amazon + Cerrado se necessrio
    camadas: list[str] = []
    ...
    try:
        async with httpx.AsyncClient(timeout=45.0, verify=False, headers=HEADERS) as client:
            ...
            logger.info(
                ...
                f"ps-2020 para {car_numero} (UF={uf})"
            )
            return {
        ...
        logger.warning(f"PRODES API indisponvel ({exc}), usando simulao.")
        return _simular_marco_ue(car_numero)


# ?Simulaes determinsticas (fallback) ?
