...
async def buscar_por_car(dados: BuscaCARRequest, db: SessaoDB, usuario: UsuarioAtual):
    ...
    if propriedade_local:
        ...
        if fresh.encontrado and fresh.status_car:
            atualizado = False
            if propriedade_local.status_car != fresh.status_car:
                propriedade_local.status_car = fresh.status_car
                atualizado = True
            if fresh.nome_propriedade and not propriedade_local.nome_propriedade:
                propriedade_local.nome_propriedade = fresh.nome_propriedade
                atualizado = True
            if fresh.area_ha and not propriedade_local.area_ha:
                propriedade_local.area_ha = fresh.area_ha
                atualizado = True
            if fresh.geojson and not propriedade_local.geojson:
                propriedade_local.geojson = fresh.geojson
                atualizado = True
            if fresh.municipio and not propriedade_local.municipio:
