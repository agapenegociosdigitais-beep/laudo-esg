...
class PropriedadeBase(BaseModel):
    numero_car: str = Field(..., description="N�mero do CAR (ex: MT-5107248-XXXX...)")
...
class BuscaCARRequest(BaseModel):
    """Request para busca de propriedade pelo n�mero do CAR."""
    numero_car: str = Field(..., description="N�mero completo do CAR")


class CARResultado(BaseModel):
    """
    Resultado retornado pelo SICAR para um n�mero de CAR.
    Inclui o ID interno do banco para uso imediato na an�lise.
    """
    id: Optional[uuid.UUID] = Field(None, description="ID interno (gerado ap�s salvar no banco)")
    numero_car: str
    estado: str
    municipio: str
    nome_propriedade: Optional[str] = None
