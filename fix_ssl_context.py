#!/usr/bin/env python3
"""Corrigir erro _ssl_context not defined no sicar_service.py"""
import paramiko
...
# Conteúdo corrigido para sicar_service.py
conteudo_corrigido = '''"""
...
STATUS_MAP = {
    ...
    "AN": "Pendente",  "NA": "Inativo",
    # valores por extenso que o SICAR às vezes retorna diretamente
    "ATIVO": "Ativo",       "PENDENTE": "Pendente",
    "CANCELADO": "Cancelado", "SUSPENSO": "Suspenso",
    "INSCRITO": "Inscrito",   "RETIFICADO": "Retificado",
    "INATIVO": "Inativo",
}
...
async def buscar_car_sicar(car_numero: str) -> dict:
    ...
    try:
        ...
        async with httpx.AsyncClient(
            timeout=30, verify=ssl_context, headers=HEADERS
        ) as client:
            ...
            # STATUS_MAP retorna string descritiva; fallback "Não informado" para códigos desconhecidos
...
# Salvar arquivo no host remoto
with open('/tmp/sicar_service_corrigido.py', 'w') as f:
    f.write(conteudo_corrigido)

# Usar SFTP para enviar o arquivo
sftp = ssh.open_sftp()
sftp.put('/tmp/sicar_service_corrigido.py', '/tmp/sicar_service_corrigido.py')
sftp.close()

# Copiar do host para o container
cmd = 'docker cp /tmp/sicar_service_corrigido.py eureka_backend:/app/app/services/sicar_service.py'
...
ssh.exec_command('rm /tmp/sicar_service_corrigido.py')
